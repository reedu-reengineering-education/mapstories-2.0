...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'PUT') {
    try {
      const step = await db.storyStepSuggestion.findFirst({
        where: {
          id: req.query.storyStepSuggestionId as string,
        },
        include: {
          content: true,
        },
      ...
      const stepToReturn = await db.storyStepSuggestion.findFirst({
        where: {
          id: step?.id,
        },
        include: {
